...
import { Search, Filter, MapPin, Navigation, Loader2, X } from 'lucide-react';
...
// Custom marker icons
const createListingIcon = () => L.divIcon({
  className: 'custom-marker',
  html: `<div style="background: #3b82f6; width: 32px; height: 32px; border-radius: 50% 50% 50% 0; transform: rotate(-45deg); display: flex; align-items: center; justify-content: center; border: 2px solid white; box-shadow: 0 2px 8px rgba(0,0,0,0.3);">
...
const createRestaurantIcon = () => L.divIcon({
  className: 'custom-marker',
  html: `<div style="background: #ef4444; width: 32px; height: 32px; border-radius: 50% 50% 50% 0; transform: rotate(-45deg); display: flex; align-items: center; justify-content: center; border: 2px solid white; box-shadow: 0 2px 8px rgba(0,0,0,0.3);">
    <span style="transform: rotate(45deg); color: white; font-size: 14px;">🍽</span>
  </div>`,
  iconSize: [32, 32],
  iconAnchor: [16, 32],
...
export default function MapSearch({
  initialCenter = { lat: 40.7128, lng: -74.006 }, // Default: NYC
  initialZoom = 13,
  radius = 10000,
  categoryId,
  onMarkerClick,
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch listings
        if (showListings) {
          const { data: listings } = await getListingsNearby(
            location,
            searchRadius,
            categoryId
          );
        ...
        // Fetch restaurants
        if (showRestaurants) {
          const { data: restaurants } = await getRestaurantsNearby(
            location,
            searchRadius
          );
          ...
          >
            <Popup>
              <div className="min-w-[200px]">
                ...
                {marker.price && (
                  <div className="text-green-600 font-bold">${marker.price.toLocaleString()}</div>
                )}
      ...
      {isLoading && (
        <div className="absolute inset-0 bg-white/80 z-[1000] flex items-center justify-center">
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-sm text-gray-600">Loading nearby items...</span>
          </div>
        </div>
      )}
...
function parseGeoJSONPoint(location: string): { lat: number; lng: number } | null {
  try {
    // PostGIS returns geography as GeoJSON in some cases
